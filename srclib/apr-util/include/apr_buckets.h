...
/*
	1. 各种常量定义、宏定义、结构体定义
		    apr_bucket_alloc_t,	apr_bucket_type_t,   apr_bucket,   apr_bucket_brigade
		    apr_bucket中的data有以下几种:	apr_bucket_heap, apr_bucket_pool, apr_bucket_mmap, apr_bucket_file, socket等
	2. 各种bucket类型定义
	3. bridage相关函数接口
	4. bucket相关函数接口
	5. bridage是一个bucket的双向环表，bucket是一个memnode的单链表
	6. 一般由bucket对应一个操作对象，如一个内存块，一个文件，
		有待确认: 一个大的file 在read之后形成一个heap buckets链表，即bridage

*/
/*
	各种bucket实际就是对各种对象的一个包装:
	1. heap bucket实际就是对一块内存区(base指针)的包装
	2. file bucket实际就是对一个文件(fd)的包装
	3. socket bucket 实际就是对一个socket的包装
*/
/**
 ...
 *
 * Bucket brigades代表了一个复杂的数据流，
 * 它可以用于IO系统间的无copy数据传输
 *
 ...
 *
 * brigade是一个bucket的环，bucket一般以brigade的member的形式出现，
 * 不过也可以在短期内独立出现
 * 
 ...
 *
 * Buckets可以用来跟多种数据存储关联，如内存、文件、mmap、输出等
 * 同时也对应多种访问接口函数，用户应该通过接口对应的宏来使用它们
 *
 ...
 * 写bucket时，先写入iovec，防止频繁写入bucket，提高性能
 */
...
struct apr_bucket_type_t {
     ...
     *      be used.
     */
     //存储在stack中的bucket数据需要保留时，实现此函数。
     //如果数据本身比bucket周期要长，如全局的、immortal、
     //heap的、则此函数只实现为apr_bucket_setaside_noop
 ...
 */
 // apr_bucket存储在堆中，生命周期由brigade控制
struct apr_bucket {
     ...
     */
     // 从base起始，第start个之后开始是data
    apr_off_t start;
    /** type-dependent data hangs off this pointer */
	//类型相关的具体数据，如apr_bucket_file、apr_socket_t等
    void *data;	
    /**
...
/** A list of buckets */
// brigade中包含一个已分配buctets列表list，以及一个未分配buctets列表bucket_alloc
struct apr_bucket_brigade {
     ...
     * doesn't like that.
     */
     //下面的参数结构体apr_bucket_list不需要名字，因为它不会被独立引用
 ...
 */
 //返回一个新的brigade,将b里面从e之后的所有buckets都放入新brigade之中
 ...
 */
 //将bb中所有buckets内容依次存入数组c中，返回数组实际长度len
 ...
 */
 // 从b中创建iovec，nvec为实际放入iovec的elem个数
 // 这在写文件和写入net I/O时很有用
 ...
 */
 //将一个字符串list写入brigade，如brigade满，则flush
 ...
 * @return APR_SUCCESS or error code
 */
 // 本函数尝试高效地处理heap buckets，如果brigade以heap bucket结尾，
 // 则先优先写入此heap bucket的剩余空间中，再考虑分配一个新的heap bucket
 ...
 */
 // 将一个文件或其中部分内容插入到brigade末尾，如插入内容大于一个bucket，则split
 // 成多个buckets
 ...
 *
 *  bucket直接从allocator，而不是从pool分配内存，是因为
 *  如果使用pool的话，bucket无法自主free自己占用的空间，
 *  需要等pool来统一free。造成内存浪费
 *  另外，allocator不能直接用于多线程
 *
 */
 //从p的alloctor中分配一个node，用于存储apr_bucket_alloc_t
 ...
 * @warning The allocator must never be used by more than one thread at a time.
 */
 //从alloctor中分配一个node，用于存储apr_bucket_alloc_t
 ...
 */
 // 当多个buckets共用同一资源时，需要等最后一个来free
 ...
 */
 // 从brigade移除bucket，并destroy此bucket
 ...
 * 简单地讲，就是有两种buckets:
 * 一种就是存储在RAM中的heap bucket，一种就是file/socket bucket
 * 本函数如果是在读heap bucket，则直接返回其起始地址；
 * 如果是在读file/socket bucket ，则要先转换成heap bucket,再read；
 * 而如果file/socket bucket太大，则需要循环转换成heap bucket，
 * 每次将未转换部分变成一个新的file/socket bucket，插入在heap bucket后面
 *
 *
 ...
 *
 * apr_bucket_read函数从bucket读取内容，返回读取内容
 * 的起始地址和长度，此函数尽力避免copy动作
 *
 * Buckets are expected to be a member of a brigade at the time they
 * are read.
 * Read时，Buckets被假设为在brigade中
 *
 ...
 * 典型地，此函数被用来循环读取一个bucket的内容，
 * 当内容读完以后，bucket就被移出brigade，然后继续读下一个bucket
 *
 ...
 * "方便"的定义取决于bucket的类型，如果是内存bucket，
 * 比如heap或immortal buckets，函数直接返回bucket内容所在
 * 的buffer指针
 *
 ...
 * 其它的buckets，如socket bucket，就无所谓len了。当从这样的
 * bucket读数据时，apr_bucket_read将从socket读一个合适长度的数据
 * socket bucket 被神奇的转换成了一个只读的heap bucket，然后在它
 * 后面insert一个新的socket bucket
 *
 ...
 * 为什么apr_bucket_read这么处理数据，我们来看它是如何
 * 循环read然后处理数据的。首先当前bucket被转变为heap bucket并返回给caller
 * 然后caller处理数据后delete heap bucket，然后转向下一个socket bucket。
 * 从外面看起来brigade就像有无限的数据。由caller决定什么时候停止read
 *
 ...
 * 一些buckets，如file bucket，大小可能是固定的。
 * 但是文件大小可能大于一次处理过程中实际存储在RAM中的大小
 * 跟socket bucket一样，file bucket会被转换成heap bucket，
 * 然后从file的offset处开始读取。在read过程中，offset会跟着往前移
 * 而且一个新的file bucket会insert到当前bucket(代表file剩余部分)的后面。
 * 如果heap bucket的大小超过文件的剩余部分，则不需要新的file bucket了，
 * 当前file bucket完成后就结束了
 *
 *
 ...
 * 上述read buckets的方式会给使用者一种错觉，就是好像对于系统来讲
 * 超大的bucket也可以一次性处理完。而实际上APR只是将超大的
 * file/socket buckets分裂成了许多个合适大小的heap buckets.
 *
 ...
 * 一个较明显的提示就是，如果buckets是在一个loop中被读取，
 * 一般表示它被慢慢地转移到RAM中，若它比RAM大，就要在APP
 * 中处理内存的使用
 *
 *
 ...
 * 本函数表示setaside()是否无操作，当bucket含有"immortal"数据时，的确如此
 *
 *
...
//以下几个notimpl函数用于判断bucket是否实现了相关函数
/**
 ...
 * 判断当前bucket在destroy前是否需要做额外的操作。
 * 如果是metadata buckets或者非bucket-type-specific structure数据，
 * 则返回true
 *
 ...
 * 这种bucket由creator全权负责，系统不对它做任何处理
 *
 */
 ...
 * to the heap
 * 存储在stack上的暂态bucket，setaside函数会将它copy到heap
 */
...
//以下几个shared函数用于处理refcounted buckets
//可以作为类型无关的通用的处理函数。
/**
...
//以下的各个create函数实际就是对file、socket等包装成一个bucket，
//然后用bucket的方式来操作file、socket等
/*
