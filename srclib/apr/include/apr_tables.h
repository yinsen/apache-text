...
// 三个主要数据结构: 
// 1. apr_array_header_t  -------->  vector数组
// 2. apr_table_entry_t  --------> array的元素为apr_table_entry_t 时，array为table
// 3. apr_table_t  --------> array为table时，总的table管理数据结构
 ...
 */
 // 将数组中的所有元素(string)串连起来形成一个新的string,并以sep插入作为元素之间的分隔符
...
// table 实际上还是一个array，只是table的每个数组元素都是apr_table_entry_t
// table的结构相当于数据库中的一张逻辑表，key就是一列的表头，
// key可以对应很多个value，value之间以","相隔。
/**
 * Make a new table.
 ...
 */
 //只插入value，将val插入到t中相同key的value中，以"," 分隔。
 ...
 */
 // ...参数表示一列给定的key值，如果为空则t中的每个元素都要执行一遍comp，
 // 如果有特定的keys，则只对这些key对应的元素调用comp
...
// 将表b合并到a中，key相同的只增加value，key不同的就增加key-value
