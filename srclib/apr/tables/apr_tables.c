...
//插入一个元素，返回新元素地址；如果数组内没有空闲节点，则新alloc，数组长度*2
...
struct apr_table_t {
     ...
     */
    apr_uint32_t index_initialized;//这里32个bit位对应表示下面32个TABLE_HASH(key)索引位置有没有entry
    int index_first[TABLE_HASH_SIZE];//table中第一个有这个key的entry
    int index_last[TABLE_HASH_SIZE];//table中最后一个有这个key的entry
    // 这样当要查找一个key的所有元素时，不必遍历整个table，
    // 而只需要遍历index_first[TABLE_HASH(key)] 与index_last[TABLE_HASH(key)]之间的元素即可
};
