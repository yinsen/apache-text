 ...
 */
#define MIN_ALLOC (2 * BOUNDARY_SIZE)//最小8k
#define MAX_INDEX   20
...
#define BOUNDARY_SIZE (1 << BOUNDARY_INDEX)//4//4k
#endif
...
struct apr_allocator_t {
     ...
     * slot 19: size 81920
     */
    apr_memnode_t      *free[MAX_INDEX];//20个链表,存储的是链表中的下一个free node?
};
...
if (index <= allocator->max_index) {
	return free[1]~free[20]中的node
}
else if (allocator->free[0]) {
	return free[0]中的node
}
return malloc;
*******************************************************************************************/
static APR_INLINE
apr_memnode_t *allocator_alloc(apr_allocator_t *allocator, apr_size_t in_size)
{
	...
	//从index开始，向下遍历free数组到max_index为止，找到有第一个节点的数组元素
	/******* 不会直接在index上new一个 *******/
	...
	/**************************************************************************************************************
	********	if成立则表示pool内有现成free节点，alloc第一个节点给node       ************************
	********	否则现有max_index内没有free节点，直接跳到malloc				************************
	***************************************************************************************************************/
        if ((node = *ref) != NULL) {
             ...
             * available index
             */
             //如果当前是在max_index链表上，且本次分配后本链表没有free节点剩余，则需要调整max_index
             //同时也将free[index]指向了下一个free节点
     ...
     */
         ...
         * any nodes on it of the requested size
         */
         //在free[0]中找一个index满足需要的node，
        ref = &allocator->free[0];
        while ((node = *ref) != NULL && index > node->index)
            ref = &node->next;

        if (node) {
            *ref = node->next;//取走了第一个节点，需要将free[0]指向下一个free节点
...
对于alloc的控制参数current_free_index，在上面alloc和这里free两个函数中的变化
并不是对称的，上面从pool中alloc时才增，这里free回pool中时才减。而pool中
的内存不是预分配，而是延时分配
*******************************************************************************************/
static APR_INLINE
void allocator_free(apr_allocator_t *allocator, apr_memnode_t *node)
{
	...
	//先处理需要free给OS，而不是pool的情况
        if (max_free_index != APR_ALLOCATOR_MAX_FREE_UNLIMITED
            && index + 1 > current_free_index) {
            //将node之后的节点链表拆下来再重新组成一个反向的freelist
            node->next = freelist;
            freelist = node;
        }

	//处理free给pool中free[1]~free[20]的情况
        else if (index < MAX_INDEX) {
            /* Add the node to the appropiate 'size' bucket.  Adjust
             * the max_index when appropiate.
             */
             //node的index大于当前的max_index，且free[index]为null，则将node放入free[index]的头节点
            if ((node->next = allocator->free[index]) == NULL
                && index > max_index) {
                max_index = index;
            }
            allocator->free[index] = node;//free给对应index的链表
	...
	//处理free给pool中free[0]的情况
        else {
            /* This node is too large to keep in a specific size bucket,
             * just add it to the sink (at index 0).
             */
             //把node之后的节点顺序插入为free[0]对应链表的第一个节点，
 ...
 */
 //apr_pool_t代表一个池，池里面的内存以apr_memnode_t为单位管理
struct apr_pool_t {
    apr_pool_t           *parent;
    apr_pool_t           *child;
    apr_pool_t           *sibling;
    apr_pool_t          **ref;//指向自身的指针；
    cleanup_t            *cleanups;
...
#if !APR_POOL_DEBUG
    apr_memnode_t        *active;//同一allocator上分配的node的循环链表，根据node的free_index从大到小排序
    apr_memnode_t        *self; /* The node containing the pool itself *///指向pool结构本身所在的apr_memnode_t节点
 ...
 * Memory allocation
 */
 /* 总体的算法就是优先从链表中最大剩余空间的节点进行分配，一直分配到空间不够，再将第二大的节点调整为第一大进行分配
active就是原先最大的那个节点*/
...
APR_DECLARE(void *) apr_palloc(apr_pool_t *pool, apr_size_t in_size)
{
	...
	/*****	2. active的空间不够则从active->next开始分配,active->next为最大，所以不需要循环找之后的节点****/
    node = active->next;
    if (size <= node_free_space(node)) {
        list_remove(node);//node需要调整位置
    }
    else {//active->next不够的话，只能alloc了
	...
	/*****	3. 从active->next或allocator分配后，将此节点变成active	****/
    node->free_index = 0;
	...
	/*****	4. 将原先的active调整到链表中合适的位置，再return mem	****/
    free_index = (APR_ALIGN(active->endp - active->first_avail + 1,
                            BOUNDARY_SIZE) - BOUNDARY_SIZE) >> BOUNDARY_INDEX;

    active->free_index = (APR_UINT32_TRUNC_CAST)free_index;
    node = active->next;
    if (free_index >= node->free_index)//active依旧是最大剩余的节点，则不需要调整位置
        return mem;
...
APR_DECLARE(void) apr_pool_clear(apr_pool_t *pool)
{
    ...
    run_cleanups(&pool->pre_cleanups);//执行用户注册的回调函数
    pool->pre_cleanups = NULL;

    /* Destroy the subpools.  The subpools will detach themselves from
     * this pool thus this loop is safe and easy.
     */
     //apr_pool_destroy()递归销毁所有child pool
    while (pool->child)
                                             ...
                                             apr_allocator_t *allocator)
{
    ...
    *newpool = NULL;
    ...
    pool = (apr_pool_t *)node->first_avail;//从first_avail开始存储pool结构体内容
	...
	//如果parent存在，则将pool置为parent的第一个child，而其它的child置为pool的sibing
...
/******************************************************************************************
分配一个node用于存储pool结构体本身，但此pool不被插入tree结构
******************************************************************************************/
 ...
 *
 * apr_psprintf首先往pool的当前block写入，从first_avail开始，
 * 如果空间不够，就会allocate一个新的block，回退之前的写入
 * 新的block在当前输出完成之前不会放入pool
 *
...
static int psprintf_flush(apr_vformatter_buff_t *vbuff)
{
    ...
    size = cur_len << 1;//每次分配都是在原有的基础上放大两倍，如果还不够，则再再次进入本函数
	...
	//把active->next置为active
	{
	 ...
	 //把原先的active重新放入链表，并重排顺序
	 {
	        free_index = (APR_ALIGN(active->endp - active->first_avail + 1,
	                                BOUNDARY_SIZE) - BOUNDARY_SIZE) >> BOUNDARY_INDEX;

	        active->free_index = (APR_UINT32_TRUNC_CAST)free_index;
	        node = active->next;//让node指向原先active的下一个，
	 ...
	 //让node指向新的active节点
        ...
        if (ps->got_a_new_node) {//如果分配了新node，原先的active就需要free，因为下面的memcpy把active的内容转移了
            active->next = ps->free;
            ps->free = active;
        }
...
APR_DECLARE(char *) apr_pvsprintf(apr_pool_t *pool, const char *fmt, va_list ap)
{
     ...
     */
     //现有active的节点为空，则调用psprintf_flush找另外的节点或分配新节点
    if (ps.node->first_avail == ps.node->endp) {
        if (psprintf_flush(&ps.vbuff) == -1) {//第一次调用，此时ps.vbuff.curpos  == ps.node->first_avail
            if (pool->abort_fn) {
                pool->abort_fn(APR_ENOMEM);
            }
	...
	//active节点剩余空间不够，psprintf_flush被调用扩大一倍空间，如果调用后还不够，会循环被调用；
	//此时buff中已经写入东西，ps.vbuff.curpos已经改变，所以在psprintf_flush中要有一个copy已写入内容到新node的操作，然后free当前node
	//所以一次写入操作都是放在一个node中，并不会跨节点存放
	//同一个节点中存的不同内容间以null为分割符
    if (apr_vformatter(psprintf_flush, &ps.vbuff, fmt, ap) == -1) {//其实&ps.vbuff就是ps的地址
        if (pool->abort_fn)
            pool->abort_fn(APR_ENOMEM);
	...
	//到这里fmt字符串已经被成功写入vbuff，即当前的active节点
    strp = ps.vbuff.curpos;
    *strp++ = '\0';
	...
	// 只有在完全成功写入后才会更改first_avail的值，
	// 在写入过程中，只以curpos暂时标记，不改变active节点的状态信息
	// 这样当active节点空间不足时，就只需要copy出已写入部分，然后直接根据原first_avail重新排序
     ...
     */
     //返回此次写入buffer的内容的起始位置
    if (!ps.got_a_new_node)
        return strp;

	//如果是新分配节点，调整新节点为active
	{
	    active = pool->active;
	    node = ps.node;
	...
	//将原先active的节点调整到合适的位置，当active变成非active时，需要更新其free_index为当前剩余空间大小，以4k为单位
	{
	    free_index = (APR_ALIGN(active->endp - active->first_avail + 1,
	                            BOUNDARY_SIZE) - BOUNDARY_SIZE) >> BOUNDARY_INDEX;
