...
/* error message.                                                           */
/*  返回值:
	1. 如果返回null，表示指令已经被执行；
	2. 如果返回DECLINE_CMD，表示本指令在多个mod中被声明，需要转交给下一个mod处理；
	3. 如果返回其它字符串，表示handler遇到问题，返回值为问题描述
*/
/*--------------------------------------------------------------------------*/
/*
...
/* 定义一个handler，当本mod收到一个document时它被激活。
    当在某些函数中，handler将数据直接发给connection时，
    他们必须保证所有HTTP header优先被发送(调用send_http_header),
    否则，将没有任何header被发送，回给client的内容只是HTTP-uncompliant*/
/*--------------------------------------------------------------------------*/
/*
...
static int x_handler(request_rec *r)
{
     ...
     * case.
     * 在发送content之前，先发送HTTP header。
     * 如果设置Content-type的话，需要通过设置r->content_type来实现
     * 我们也需要启动一个timer以让server知道连接是否中断
     *
