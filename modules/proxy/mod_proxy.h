...
//proxy_worker用于管理origin server的配置和连接参数
//有两个默认worker: forward proxy(正向代理) 和reverse proxy(反向代理)，它们有固定的配置
//当没有其它合适的worker时，就会启用这两个默认worker
//worker不使用长连接和连接池，它到origin server的连接都是每个req一个新的连接
struct proxy_worker {
...
struct proxy_balancer {
    ...
    /*用户对同一内容的多次请求，可能被转发到了不同的后端服务器，
    用户发出一请求被分配至服务器A，保存了一些信息在session中，
    该用户再次发送请求被分配到服务器B，若要让B可以正常服务，
    则需要使用分布式session或分布式缓存，或者保证用户的请求都被发给A,
    这就需要实现粘滞会话，sticky session*/
