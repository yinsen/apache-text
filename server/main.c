...
static process_rec *init_process(int *argc, const char * const * *argv)
{
	...
	//初始化global pool、tag pool、signal pool、mutex等
...
//初始化、申请内存池、配置、最后启动mpm
//此main函数就相当于mpm启动的公共部分。
//main+ap_mpm_run就相当于fastcgi的pm主进程
int main(int argc, const char * const argv[])
{
	...
	//创建process_rec结构体，并创建其global pool、conf pool
     ...
     */
     //将参数放入opt结构体，并将opt结构体放入pcommands pool
    ...
    while ((rv = apr_getopt(opt, AP_SERVER_BASEARGS, &c, &optarg))
        ...
        case 'c':
            ...
            *new = apr_pstrdup(pcommands, optarg);//将optarg copy到pool
            break;
	...
	//之后将apache的运行完全交给mpm。如mpm出错则退出;
	//如果期间发送了重启Apache的命令，则会退出ap_mpm_run，重新进入本次循环，重新配置。
	//所以当前for(;;)也称之为Apache重启循环
	//相当于配置结束，主进程(即管理进程)进入下一阶段
